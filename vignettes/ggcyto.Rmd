...
p <- ggcyto(tbdata, aes(x = CD4, y = CD8), subset = "3+") 
...
p + geom_stats("4+", type = "count", size = 6,  color = "white", fill = "black", adjust = 0.3)
```
...
```{r}
#'subset' is ommitted
p <- ggcyto(tbdata, aes(x = CD4, y = CD8)) + geom_hex(bins = 64) + myPars + geom_gate(c("4+", "8+"))
p
```
...
```{r}
p <- ggcyto(tbdata, aes(x = CD4, y = CD8), subset = "3+") + geom_hex(bins = 64) + geom_gate() + geom_stats() + myPars
p 
```
